    ...
    with open(sys.argv[1], "r") as f:
        ...
        for record in reader:
            ...
            if record.isStart():
                ...
                # print(f"{record.entry},{len(record.data)}", end="")
                entry = entries.get(record.entry)
                if entry is None:
                    print("<ID not found>")
                    continue
                # print(f"{entry.name},{entry.type},{timestamp},", end="")
                output = dict(
                    entry=record.entry,
                    len=len(record.data),
                ...
                try:
                    ...
                    if entry.name == "systemTime" and entry.type == "int64":
                        ...
                        date_string = "{:%Y-%m-%d %H:%M:%S.%f}".format(dt)
                        output["systemtime"] = date_string
                        continue

                    if entry.type == "double":
                        v = record.getDouble()
                        if math.isfinite(v):
                            output["data_double"] = v
                        else:
                            output["unrepresentable"] = repr(v)
                    elif entry.type == "int64":
                        output["data_int"] = record.getInteger()
                    elif entry.type in ("string", "json"):
                        output["data_str"] = record.getString()
                    elif entry.type == "msgpack":
                        output["data_str"] = json.dumps(record.getMsgPack())
                    elif entry.type == "boolean":
                        output["data_bool"] = record.getBoolean()
                    elif entry.type == "boolean[]":
                        output["data_str"] = json.dumps(list(record.getBooleanArray()))
                    elif entry.type == "double[]":
                        output["data_str"] = json.dumps(list(record.getDoubleArray()))
                    elif entry.type == "float[]":
                        output["data_str"] = json.dumps(list(record.getFloatArray()))
                    elif entry.type == "int64[]":
                        output["data_str"] = json.dumps(list(record.getIntegerArray()))
                    elif entry.type == "string[]":
                        output["data_str"] = json.dumps(list(record.getStringArray()))
                    elif entry.type == "float":
                        v = record.getFloat()
                        if math.isfinite(v):
                            output["data_double"] = v
                        ...
                        output["data_str"] = record.getString()
                    elif entry.type.startswith("struct:") or entry.type == "raw":
                        output["data_str"] = base64.b64encode(
                            bytes(record.data)
                        ).decode("ascii")
